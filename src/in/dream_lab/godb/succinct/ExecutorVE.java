...
public class ExecutorVE {
	...
	private static void init(String Arguments){
		...
		ExecutorVE.Step.Type previousStepType = ExecutorVE.Step.Type.EDGE;
		for(String _string : arguments.split(Pattern.quote("//"))[0].split(Pattern.quote("@")) ){
			if(_string.contains("?")){
				if(previousStepType == ExecutorVE.Step.Type.EDGE)
					path.add(new Step(ExecutorVE.Step.Type.VERTEX,null, null, null));
				previousStepType = ExecutorVE.Step.Type.EDGE;
				String[] _contents = _string.split(Pattern.quote("?")); 
				String p = null ;
				Object v = null ;
				ExecutorVE.Step.Direction d = (_contents[0].equals("out") ) ? ExecutorVE.Step.Direction.OUT : ExecutorVE.Step.Direction.IN;
				...
				path.add(new Step(ExecutorVE.Step.Type.EDGE, d, p, v));
			}
			else{
				previousStepType = ExecutorVE.Step.Type.VERTEX;
				...
				path.add(new Step(ExecutorVE.Step.Type.VERTEX,null, p, v));
			}
...
//			IVertex<MapValue, MapValue, LongWritable, LongWritable> currentVertex = getSubgraph().getVertexById(new LongWritable(vertexMessageStep.vertexId));
			...
			if( nextStep.type == ExecutorVE.Step.Type.EDGE ) {
				
				if ( nextStep.direction == ExecutorVE.Step.Direction.OUT ) {
...
//				}
			...
			else if ( nextStep.type == ExecutorVE.Step.Type.VERTEX ) {
